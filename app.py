...
# Códigos corretos da classificação 315 para os 9 grupos do IPCA
IPCA_GRUPOS_IDS = "7170,7445,7486,7558,7625,7660,7712,7766,7786"

# Cores por nome normalizado (sem acento, minúsculo) — aplicadas após strip do prefixo
IPCA_GRUPOS_CORES_NORM = {
    "alimentacao e bebidas":      "#d97706",
    "habitacao":                   "#0891b2",
    "artigos de residencia":       "#64748b",
    "vestuario":                   "#ec4899",
    "transportes":                 "#7c3aed",
    "saude e cuidados pessoais":   "#16a34a",
...
def _limpa_nome_grupo(nome: str) -> str:
    """Remove prefixo numérico ('1.Alimentação' → 'Alimentação') e espaços."""
    import re
    nome = str(nome).strip()
...
def grupos_bar_fig(df_grupos, ultimo_mes):
    df_m = df_grupos[
        ...
        (df_grupos["grupo_id"].isin(["7170","7445","7486","7558","7625","7660","7712","7766","7786"]))
    ].copy()
    if df_m.empty:
        return go.Figure()
    ...
    if df_grupos_mensal.empty:
        ...
        if pd.isna(ultimo_mes):
            ...
            df_ult = df_grupos_mensal[
                ...
                (df_grupos_mensal["grupo_id"].isin(["7170","7445","7486","7558","7625","7660","7712","7766","7786"]))
            ...
            if not df_grupos_acum.empty:
                ...
                df_acum_u = df_grupos_acum[
                    ...
                    (df_grupos_acum["grupo_id"].isin(["7170","7445","7486","7558","7625","7660","7712","7766","7786"]))
